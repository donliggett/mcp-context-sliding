...
export class TokenCounter {
    calibration;
    calibrating = null;
    constructor(initialRatio, source = 'default') {
        this.calibration = {
            ratio: initialRatio,
            model: '',
    ...
    estimateAll(texts, separatorTokens = 1) {
    ...
    async calibrate(llm) {
        ...
        this.calibrating = (async () => {
            ...
            const unit = 'The quick brown fox jumps over the lazy dog while the committee ' +
    ...
    truncateToBudget(text, budgetTokens) {
        ...
        if (this.estimate(text) <= budgetTokens)
        ...
        while (lo < hi) {
            ...
            if (this.estimate(text.slice(0, mid)) <= budgetTokens)
                lo = mid;
            else
                hi = mid - 1;
        }
        ...
        if (paragraph > 0)
            cut = offset + paragraph;
        else if (sentence > 0)
            cut = offset + sentence + 1;
        else if (space > 0)
            cut = offset + space;
