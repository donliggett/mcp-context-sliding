...
const DEFAULT_INSTRUCTION = 'Summarize the material below. Preserve concrete details — names, numbers, ' +
...
export class Summarizer {
    ...
    async summarize(text, options) {
        ...
        if (this.llm.enabled) {
            try {
                ...
                const reason = err instanceof LlmUnavailableError ? err.message : String(err);
    ...
    async llmSummarize(text, options) {
        ...
        const perSliceTarget = Math.max(120, Math.floor((options.targetTokens * 2) / Math.max(1, slices.length)));
        const partials = [];
        for (const slice of slices) {
            ...
            partials.push(await this.callModel(slice.text, {
    ...
    async callModel(text, options) {
        ...
        const result = await this.llm.chat([
            {
                role: 'system',
                content: 'You are a summarization engine. You output only the summary itself — ' +
...
export function splitSentences(text) {
    ...
    if (normalized.length === 0)
        return [];
    const parts = normalized.split(/(?<!\b(?:e\.g|i\.e|vs|etc|Dr|Mr|Mrs|Ms|Prof|Fig|No|approx)\.)(?<!\b[A-Z])(?<=[.!?])\s+(?=[A-Z0-9"'(\[])/);
    return parts.map((p) => p.trim()).filter((p) => p.length > 0);
}
function termsOf(text) {
    return (text.toLowerCase().match(/[a-z0-9_$]{3,}/g) ?? []).filter((w) => !/^\d+$/.test(w));
}
