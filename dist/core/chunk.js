...
function updateTrail(trail, level, text) {
 ...
 */
function hardSplit(block, counter, chunkTokens) {
    const out = [];
...
export function chunkText(text, counter, options) {
    ...
    const emit = () => {
        ...
        if (overlapTokens > 0) {
            ...
            for (let i = pending.length - 1; i >= 0; i--) {
                ...
                if (carriedTokens + piece.tokens > overlapTokens)
                    break;
    ...
    for (const block of blocks) {
        if (block.headingLevel > 0) {
            ...
            if (pendingTokens > chunkTokens * 0.5)
                emit();
        }
