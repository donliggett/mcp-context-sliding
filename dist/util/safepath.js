...
export class IngestSandbox {
    ...
    async resolveFile(input) {
        if (!this.enabled) {
            throw new InvalidError('file ingestion is disabled — start the server with --ingest-root <dir> to enable it, ' +
                'or pass the content directly via the text argument');
        }
        if (input.includes('\0'))
            throw new InvalidError('path rejected');
        ...
        if (!this.roots.some((root) => this.within(real, root))) {
            throw new InvalidError(`access denied: ${input} is outside the allowed ingest directories ` +
                `(${this.roots.join(', ')})`);
        }
        const stats = await fs.stat(real);
        if (!stats.isFile())
            throw new InvalidError(`${input} is not a regular file`);
        return real;
    }
}
