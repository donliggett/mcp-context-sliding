...
import { loadConfig, ConfigError, HELP_TEXT } from './config.js';
 ...
 */
async function calibrateInBackground(counter, llm, config, auditor) {
...
async function serveHttp(ctx, config) {
    ...
    if (bindsPublicly) {
        ctx.auditor.error('WARNING: bound to a non-loopback address with no authentication. Anyone who can ' +
            'reach this port can read and modify every stored session and document.', { host: config.httpHost });
    }
    const shutdown = () => {
...
function serveStdioTransport(ctx) {
...
async function main() {
    ...
    void calibrateInBackground(counter, llm, config, auditor).catch((err) => {
