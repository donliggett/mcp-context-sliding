...
import { ENTRY_ROLES, type Entry } from '../store/sessions.js';
...
function renderEntry(entry: Entry, now: number): string {
  const flags = [entry.id];
  if (entry.pinned) flags.push('pinned');
  flags.push(entry.role);
  flags.push(relativeTime(entry.createdAt, now));
  if (entry.tags.length > 0) flags.push(`#${entry.tags.join(' #')}`);
  return `[${flags.join(' · ')}]\n${entry.text}`;
}
...
export function registerSessionTools(server: McpServer, ctx: ToolContext): void {
  defineTool(server, ctx, {
    name: 'context_open',
    description:
      'Start or resume a named memory session. Call this ONCE at the beginning of any task ' +
  ...
  defineTool(server, ctx, {
    name: 'context_append',
    description:
      'Record something worth remembering into the session. Use this as you go — after ' +
    ...
    schema: z.object({
      ...
      role: z
        .enum(ENTRY_ROLES as [string, ...string[]])
        .optional()
    ...
    async handler(args, { sessions, counter, config }) {
      ...
      const entry = await session.append(
        {
          role: (args.role ?? 'note') as Entry['role'],
          text: args.text,
          tags: args.tags ?? [],
      ...
      return {
        body:
          `Stored ${entry.id} (${entry.tokens} tokens${entry.pinned ? ', pinned' : ''}).\n` +
  ...
  defineTool(server, ctx, {
    name: 'context_recall',
    description:
      'Retrieve the most relevant stored entries for a query, packed to fit a token budget. ' +
    ...
    async handler(args, { sessions, config }) {
      ...
      const retrievables: Array<Retrievable & { entry: Entry }> = live.map((entry) => ({
        id: entry.id,
        text: entry.text,
      ...
      if (packed.selected.length === 0) {
        return {
          body:
            `No entries matched${args.query ? ` "${args.query}"` : ''} within ${budget} tokens. ` +
      ...
      const header =
        `${packed.selected.length} of ${live.length} entries` +
  ...
  defineTool(server, ctx, {
    ...
    description:
      'Fold older entries into a single summary to free budget, keeping the most recent ' +
    ...
    async handler(args, { sessions, summarizer, counter }) {
      ...
      if (candidates.length < 2) {
        return {
          body:
            `Nothing to compact: ${pluralize(candidates.length, 'entry', 'entries')} eligible ` +
      ...
      if (args.dry_run) {
        return {
          body:
            `Dry run — would fold ${pluralize(candidates.length, 'entry', 'entries')} ` +
      ...
      const summary = await summarizer.summarize(material, {
        targetTokens,
        instruction:
          'These are working notes from an ongoing task. Produce a condensed record that ' +
      ...
      const note =
        summary.method === 'extractive' && summary.fallbackReason
      ...
      return {
        body:
          `Compacted ${pluralize(candidates.length, 'entry', 'entries')} (${reclaimable} tokens) ` +
  ...
  defineTool(server, ctx, {
    name: 'context_status',
    description:
      'Report how full a session is, what is pinned, and whether compaction is advisable. ' +
  ...
  defineTool(server, ctx, {
    name: 'context_update',
    description:
      'Pin, unpin, or delete a single entry by its id. Pin what must survive compaction; ' +
    ...
    async handler(args, { sessions }) {
      ...
      const changes: { pinned?: boolean; deleted?: boolean } = {};
      if (args.pinned !== undefined) changes.pinned = args.pinned;
      if (args.delete !== undefined) changes.deleted = args.delete;
  ...
  defineTool(server, ctx, {
    ...
    description:
      'List stored sessions, newest first. Use this to find a session id from earlier work ' +
