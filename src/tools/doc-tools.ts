...
import { Bm25Index, packToBudget, type Retrievable } from '../core/retrieval.js';
import { pluralize, relativeTime } from '../util/format.js';
import { InvalidError, LimitError } from '../util/errors.js';
import type { DocChunk } from '../store/documents.js';
...
export function registerDocumentTools(server: McpServer, ctx: ToolContext): void {
  defineTool(server, ctx, {
    name: 'doc_ingest',
    description:
      'Load a large text into the store and split it into addressable chunks. Use this ' +
    ...
    async handler(args, { documents, counter, config, ingest }) {
      ...
      if (args.file_path) {
        ...
        if (stats.size > config.maxIngestChars) {
          throw new LimitError(
            `${args.file_path} is ${stats.size} bytes, over the ${config.maxIngestChars} ingest limit`,
          );
        }
      ...
      if (content.trim().length === 0) throw new InvalidError('content is empty');
      ...
      return {
        body:
          `Ingested "${doc.meta.title}" as ${doc.meta.id}\n` +
  ...
  defineTool(server, ctx, {
    name: 'doc_outline',
    description:
      'Show the structure of an ingested document: every chunk with its index, heading and ' +
    ...
    async handler(args, { documents, summarizer }) {
      ...
      if (args.summarize) {
        for (const chunk of chunks) {
          if (chunk.summary) continue; // Cached from a previous call.
      ...
      return {
        body:
          `${doc.meta.title} (${doc.meta.id}) — ${pluralize(doc.meta.chunkCount, 'chunk')}, ` +
  ...
  defineTool(server, ctx, {
    name: 'doc_window',
    description:
      'Read a range of chunks verbatim. This is the sliding window: ask for chunks 0-2, ' +
    ...
    async handler(args, { documents, config }) {
      const doc = await documents.load(args.doc_id);
      if (doc.chunks.length === 0) throw new InvalidError(`document ${doc.meta.id} has no chunks`);
      ...
      if (start >= doc.chunks.length) {
        return {
          body:
            `End of document: ${doc.meta.id} has ${pluralize(doc.chunks.length, 'chunk')} ` +
      ...
      for (let i = start; i <= hardEnd; i++) {
        ...
        if (taken.length > 0 && used + chunk.tokens > budget) break;
        taken.push(chunk);
        used += chunk.tokens;
      }
      ...
      const header =
        `${doc.meta.title} — chunks ${taken[0]!.index}-${taken[taken.length - 1]!.index} ` +
  ...
  defineTool(server, ctx, {
    name: 'doc_search',
    description:
      'Find the chunks of a document most relevant to a query and return them verbatim, ' +
    ...
    async handler(args, { documents, config }) {
      ...
      type Hit = Retrievable & { docId: string; docTitle: string; chunk: DocChunk };
      ...
      if (packed.selected.length === 0) {
        return {
          body:
            `No chunks matched "${args.query}" across ${pluralize(metas.length, 'document')}. ` +
      ...
      return {
        body:
          `${packed.selected.length} matching ${packed.selected.length === 1 ? 'chunk' : 'chunks'} ` +
  ...
  defineTool(server, ctx, {
    name: 'doc_summarize',
    description:
      'Summarize a whole document or a range of its chunks. Use this to get the gist of ' +
    ...
    async handler(args, { documents, summarizer }) {
      ...
      if (from > to) throw new InvalidError('from must not be greater than to');

      const selected = doc.chunks.filter((c) => c.index >= from && c.index <= to);
      if (selected.length === 0) throw new InvalidError(`no chunks in range ${from}-${to}`);

      const result = await summarizer.summarize(selected.map((c) => c.text).join('\n\n'), {
        targetTokens,
        instruction:
          'Summarize this document faithfully. Keep specific facts, names, numbers, ' +
          'identifiers and conclusions. Do not speculate about content not shown.',
      });

      if (whole) await documents.updateMeta(doc.meta.id, { summary: result.text });
      ...
      return {
        body:
          `${doc.meta.title} — chunks ${from}-${to} of 0-${doc.chunks.length - 1}\n\n` +
          result.text +
          note,
  ...
  defineTool(server, ctx, {
    ...
    async handler(_args, { documents }) {
      ...
      if (metas.length === 0) return { body: 'No documents ingested yet.' };
  ...
  defineTool(server, ctx, {
    name: 'doc_forget',
    description:
      'Permanently delete an ingested document and everything derived from it. Use when a ' +
